...
class SecurityMiddleware
{
    ...
    private function isSuspiciousRequest(Request $request): bool
    {
        ...
        // Also check all request parameters (GET, POST, etc.)
        $allParams = array_merge(
            $request->all(),
            $request->query->all(),
            $request->request->all()
        );
        $paramString = json_encode($allParams);
        ...
        foreach ($suspiciousPatterns as $pattern) {
            if (preg_match($pattern, $url) ||
                ...
                preg_match($pattern, $paramString)) {
                return true;
            }
        }
    ...
    private function applyRateLimiting(Request $request): bool
    {
        ...
        // Note: Login rate limiting is handled by the authentication controller
